...
function findStage(map, keywords) {
...
function sum(arr) { return arr.reduce((s, d) => s + dealValue(d), 0); }
...
async function buildData(params) {
  ...
  // Etapas ordenadas
  const stagesArr = (stagesRaw?.deal_stages || []).sort((a, b) => (a._position || 0) - (b._position || 0));
  const stages = {}; // id => name
  const stagePos = {}; // id => position
  ...
  // Etapas chave
  const sidAgendada  = findStage(stageIdMap, ['agendamento', 'agendada', 'agend']);
  const sidRealizada = findStage(stageIdMap, ['realizada', 'realiz']);
  const sidNegoc     = findStage(stageIdMap, ['negociação', 'negociacao', 'negoc']);
  const sidGanho     = findStage(stageIdMap, ['ganho', 'fechado ganho']);
  const sidPerdido   = findStage(stageIdMap, ['perdido', 'fechado perdido']);
  ...
  const posGanho     = stagePos[sidGanho]     ?? 999;
  ...
  const lost = filtered.filter(d => d.win === false && d.win !== null && d.win !== undefined);
  const open = filtered.filter(d => d.win === null || d.win === undefined);

  // Contagem acumulada
  const getPos = d => stagePos[d.deal_stage_id] ?? -1;
  ...
  const valNegoc  = sum(open.filter(d => getPos(d) >= posNegoc));
  const valPipeline = sum(open);

  // Funil visual acumulado
  ...
  filtered.forEach(d => {
    ...
    if (d.win === false && d.win !== null) byOwner[name].lost++;
  });
  ...
  const [mktFunnel, leadsHoje, leadesMes] = await Promise.all([
  ...
  return {
    ...
    comercial: {
      ...
      pipeline_aberto:    Math.round(valPipeline * 100) / 100,
